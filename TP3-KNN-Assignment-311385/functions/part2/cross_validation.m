...
TP_rate_fold = zeros(F_fold,size(params.k_range,2));
FP_rate_fold = zeros(F_fold,size(params.k_range,2));

for folds=1:F_fold
    [X_train, Y_train, X_test, Y_test] = split_data(X, y, valid_ratio);
    [TP_rate_fold(folds,:),FP_rate_fold(folds,:)] = knn_ROC( X_train, Y_train, X_test, Y_test,  params );
end

avgTP = mean(TP_rate_fold,1) ;
avgFP = mean(FP_rate_fold,1) ;
stdTP = std(TP_rate_fold,1,1);
stdFP = std(FP_rate_fold,1,1); 
